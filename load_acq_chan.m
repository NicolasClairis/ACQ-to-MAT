...
%  Modified on 20-DEC-2016 by Samantha Kumarasena (skumarasena@partners.edu) 
%  to allow the program to read and save only the channels specified by the 
%  user.
%  This alleviates memory issues when attempting to load larger files. 
%  Functions modified: load_acq (new version: load_acq_chan)
%                      read_acq (new version: read_acq_chan)

function acq = load_acq_chan(filename, chindices, chan_by_chan)
   ...
   %  read data
   %
   acq.data = read_acq_chan(fid, acq.hdr, chindices, chan_by_chan);
...
%{
   Identical to read_acq_chan() (the function above), except that this
   function only saves a single channel's data. 
   
   Channel number specified by chindex.
%}
   
   %---------------------------------------------------------------------
function data = read_acq_chan(fid, hdr, chindices, chan_by_chan)
	...
	length(unique(sample_divider))==1 && unique(sample_divider)==1
      ...
      if length(unique(sample_divider))==1 && unique(sample_divider)==1
        ...
        for i = 1:length(chindices)
            ...
            %  Need to skip the rest of chan, in order to read each sample point
            %
            skip_chan = size_all_chan_per_sample - hdr.per_chan_type(chindex).size;

            if hdr.per_chan_type(chindex).type == 1		% double

               tmp=fread(fid,hdr.per_chan_data(chindex).buf_length,'double',skip_chan);
               ...
               %  for integer data
               %
               tmp=fread(fid,hdr.per_chan_data(chindex).buf_length,'int16',skip_chan) ...
            * hdr.per_chan_data(chindex).ampl_scale ...
            + hdr.per_chan_data(chindex).ampl_offset;
            end
